...
- Loadtest flow (`loadtest` profile): mock gRPC analysis + synthetic nurse-stream pump + runtime/scenario APIs under `/api/loadtest/**` (`src/main/resources/application-loadtest.yaml`, `src/main/java/youzi/lin/server/config/LoadTestProperties.java`, `src/main/java/youzi/lin/server/controller/LoadTestRuntimeController.java`, `src/main/java/youzi/lin/server/controller/LoadTestNurseScenarioController.java`).
...
- Loadtest-only components are profile-gated with `@Profile("loadtest")`; keep them isolated from default runtime paths (`LoadTestNurseDataPump`, `LoadTestNurseScenarioService`, `LoadTestRuntimeController`).
...
- Change loadtest behavior (mock gRPC latency, nurse pump, scenario tooling): `src/main/resources/application-loadtest.yaml`, `src/main/java/youzi/lin/server/config/LoadTestProperties.java`, `src/main/java/youzi/lin/server/controller/LoadTest*`, `src/main/java/youzi/lin/server/service/LoadTest*`.
...
- One glob search was run for common AI instruction files; matches found: `AGENTS.md`, `tools/loadtest/README.md`, `tools/nurse_scenario_tk/README.md`.
